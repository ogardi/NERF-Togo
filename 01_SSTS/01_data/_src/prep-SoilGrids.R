####################################################################
# SSTS/data/prep-SoilGrids.R: importing SoildGrids Data
# ------------------------------------------------------------------
# Bern University of Applied Sciences
# Oliver Gardi, <oliver.gardi@bfh.ch>
# 11 March 2020

# Data obtained from: https://files.isric.org/soilgrids/data/recent/
...
attr <- merge(cats, attr[,c("Number", "Group", "WRB_group", "R", "G", "B")], by.x = "ID", by.y = "Number")
cats[["Soil Type"]] <- attr$Group
levels(soil.types) <- cats
levelplot(soil.types, col.regions=paste0("#", as.hexmode(tmp$R),as.hexmode(tmp$G),as.hexmode(tmp$B)), xlab="", ylab="") +
...
tmp$IPCC <- NA
tmp$IPCC[tmp$WRB_group %in% c("Leptosols","Vertisols","Kastanozems","Chernozems","Phaeozems","Luvisols","Alisols","Albeluvisols","Solonetz","Calcisols","Gypsisols","Umbrisols","Cambisols","Regosols")] <- "HAC"
tmp$IPCC[tmp$WRB_group %in% c("Acrisols","Lixisols","Nitisols","Ferralsols","Durisols")] <- "LAC"
